...
def apply_shift_numpy(
    ...
    Args:
        frame: 2D array (rows, cols).
        row_shift: Row shift in pixels.
        col_shift: Col shift in pixels.
        view_r: Row coordinates grid (default: 0..rows-1).
        view_c: Col coordinates grid (default: 0..cols-1).
    ...
    # Replace NaN in frame with 0 for interpolation
...
def apply_shifts_batch(
    ...
    Args:
        frames: Input frames, shape (H, W, N).
        row_shifts: Per-frame row shifts, shape (N,).
        col_shifts: Per-frame col shifts, shape (N,).
        out_h: Output height.
        out_w: Output width.
        view_r_start: Starting row coordinate of the output grid (typically -maxshift).
        view_c_start: Starting col coordinate of the output grid (typically -maxshift).
        device: Torch device (default: CPU). GPU gives ~5x over CPU.
        batch_size: Frames per batch (reduce if GPU OOM).
    ...
    # Build base grid in normalized [-1, 1] coords (shared across all frames)
    ...
    for start in range(0, n_frames, batch_size):
        ...
        # Per-frame shift offsets in normalized coords
        ...
        # Build per-frame grids: (N, out_h, out_w, 2)
        ...
        oob = (grid[..., 0] < -1) | (grid[..., 0] > 1) | (grid[..., 1] < -1) | (grid[..., 1] > 1)
        out[:, 0][oob] = float("nan")
