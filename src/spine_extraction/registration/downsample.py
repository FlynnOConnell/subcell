...
def downsample_time(data: np.ndarray, ds_time: int) -> np.ndarray:
    ...
    Args:
        data: Array of shape (..., T) where T is the temporal dimension (last axis).
        ds_time: Number of halvings. Total factor = 2^ds_time.
...
def downsample_space(data: np.ndarray, ds_times: int = 2) -> np.ndarray:
    ...
    Args:
        data: 2D or 3D array with spatial dims as first two axes.
        ds_times: Number of halvings in each spatial dimension.
