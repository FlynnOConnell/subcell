...
def minmax_downsample(
    ...
    Args:
        x: (N,) time values.
        y: (N,) signal values (NaN-safe).
        n_out: target number of output points (actual output is ~n_out).
...
class TracePanel:
    ...
    def __init__(self, data: ExtractionData, n_display_points: int = 4000,
                 dark: bool = True):
        self._data = data
        ...
        # Build time axis for current trial
    ...
    def update_traces(self, selected: list[int]) -> None:
        ...
        # Limit to MAX_TRACES, keeping lowest indices
        ...
        if not selected:
            ...
            self._fig.renderers = [r for r in self._fig.renderers
                                   if not hasattr(r, "_trace_tag")]
            self._sources.clear()
        ...
        # Pre-compute all trace data and y-range BEFORE touching the figure
        colors = _COLORS
        tick_positions = []
        tick_labels = {}
        global_y_min = np.inf
        ...
        for i, src_idx in enumerate(selected):
            ...
            t_ds, y_ds = minmax_downsample(self._t_full, y_full,
                                           self._n_display_points)
            y_ds = y_ds + offset
        ...
        # Compute final y-range
        margin = (global_y_max - global_y_min) * 0.05 if global_y_max > global_y_min else 0.5
        y_lo = global_y_min - margin
        y_hi = global_y_max + margin

        # Batch all figure updates so Bokeh renders in a single pass
        doc = self._fig.document
        if doc:
            doc.hold("combine")

        try:
            # Set y-range FIRST so new renderers appear within bounds
            ...
            for src_idx, t_ds, y_ds in trace_data:
                if selection_changed:
                    ...
                    r = self._fig.line(
                        "t", "y", source=cds,
                        line_color=color, line_width=1.2, line_alpha=0.85,
                    )
                    r._trace_tag = True
                else:
            ...
            # Build JS lookup: find closest offset → source number
            pairs = ", ".join(
                f"[{offset}, {src_idx}]"
                for offset, src_idx in tick_labels.items()
            )
            self._fig.yaxis.formatter = CustomJSTickFormatter(code=f"""
                var pairs = [{pairs}];
                var best = '';
                var best_dist = 1e9;
