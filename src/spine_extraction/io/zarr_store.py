...
class ExperimentStore:
    ...
    def save_alignment_data(self, trial_idx: int, adata: AlignmentData) -> None:
        ...
        for name in ("motion_r", "motion_c", "motion_ds_r", "motion_ds_c", "rec_neg_err"):
    ...
    def save_avg_image(
        self, trial_idx: int, channel: int, image: np.ndarray
    ) -> None:
        """Save an 8-bit average image for a channel."""
        grp = self._trial_group(trial_idx).require_group("avg_images")
        grp.create_array(f"ch{channel}_8bit", data=image.astype(np.uint8), overwrite=True)
    ...
    def save_activity_image(self, trial_idx: int, image: np.ndarray) -> None:
        ...
        grp = self._trial_group(trial_idx)
        grp.create_array("activity_image", data=image.astype(np.float32), overwrite=True)
    ...
    def load_registered_raw(self, trial_idx: int) -> np.ndarray:
        """Load full-resolution registered data for a trial."""
    ...
    def save_extraction_results(
        ...
        if global_f is not None:
            grp.create_array("global_F", data=global_f.astype(np.float32), overwrite=True)
