...
Usage::
    ...
    from subcell.visualization import ExtractionViewer
...
def _pick_mat(chooser, result: list) -> None:
    """Tk callback: pick a .mat file, then close the chooser."""
    chooser.withdraw()
...
def _pick_zarr(chooser, result: list) -> None:
    """Tk callback: pick a .zarr folder, then close the chooser."""
    chooser.withdraw()
...
def _browse_file() -> str | None:
    """
    Ask the user for a .mat file or .zarr folder.

    Returns
    -------
    str or None
        Chosen path, or None if tkinter is unavailable or the user cancelled.
    """
    if tk is None:
        logger.debug("tkinter is unavailable; cannot open a file chooser")
        return None
    ...
    tk.Button(
        btn_frame, text="  .mat File  ", command=partial(_pick_mat, chooser, result)
    ).pack(side=tk.LEFT, padx=8)
    tk.Button(
        btn_frame, text="  .zarr Folder  ", command=partial(_pick_zarr, chooser, result)
    ).pack(side=tk.LEFT, padx=8)
    ...
    def _do_load(self, path: Path) -> None:
        ...
        try:
            ...
            self._image_panel = ImagePanel(
                data,
                on_source_tapped=self._on_source_tapped,
                dark=self._dark,
            )
            self._roi_list = ROIListPanel(
                data,
                on_selection_change=self._on_list_selection_changed,
            )
