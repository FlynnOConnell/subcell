...
class ImagePanel:
    ...
    def __init__(
        ...
        self._fig.toolbar.active_drag = None
        ...
        self._mean_source = ColumnDataSource(
            data={
        ...
        self._overlay_source = ColumnDataSource(
            data={
        ...
        if data.n_sources > 0:
            self._markers_source = ColumnDataSource(
                data={
        ...
        for src_idx in self._selected:
            ...
            for ch, color_val in enumerate([r, g, b]):
                rgba[mask, ch] = np.clip(
                    ...
                    + (color_val * alpha[mask].astype(np.float32) / 255).astype(
                        np.int16
                    ),
                    0,
                    255,
