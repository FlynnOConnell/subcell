...
def spatiotemporal_nms(
    ...
    ----------
    movie : np.ndarray
        Filtered movie, shape (rows, cols, time).
    tau_frames : float
        Decay time in frames (used to skip uncertain end frames).
    nan_mask : np.ndarray, optional
        Boolean mask of NaN positions, same shape as movie.
    ...
    tuple[np.ndarray, np.ndarray ], optional
        ...
        - activity_image
        Observation-normalized sum of squared local maxima
    ...
    for start in range(1, end, chunk_size):
        ...
        if np.any(nan_cur):
            nan_neighbor = (
                maximum_filter(nan_cur.astype(np.float32), size=(3, 3, 1)) > 0
            )
...
def iterative_nms_peaks(
    ...
    ----------
    image : np.ndarray
        2D array (activity image with local baseline subtracted).
    dilation_size : int
        Size of dilation kernel to suppress around found peaks.
...
def apply_density_threshold(
    ...
    image : np.ndarray
        Activity image.
    peak_mask : np.ndarray
        Boolean mask of candidate peaks.
    max_density : float
        Maximum synapses per valid pixel.
    n_time_points : int, optional
        Number of time points (for time-dependent threshold).
    align_hz : float, optional
        Alignment sampling rate.
