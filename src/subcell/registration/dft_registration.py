...
def dft_registration_clipped_numpy(
    ...
    ----------
    buf1ft : np.ndarray
        FFT of reference image (complex ndarray).
    buf2ft : np.ndarray
        FFT of image to register (complex ndarray).
    usfac : int
        Upsampling factor.
    clip : int | tuple[int, int]
    ...
    if usfac == 0:
        ...
        return RegistrationResult(
            error=error, diffphase=diffphase, row_shift=0.0, col_shift=0.0
        )
    ...
    c_start = n + 1 - n // 2 - 1
    c_end = c_start + n
    cc[r_start:r_end, c_start:c_end] = np.fft.fftshift(buf1ft) * np.conj(
    ...
    return RegistrationResult(
        error=error,
        diffphase=diffphase,
        row_shift=float(row_shift),
        col_shift=float(col_shift),
    )
...
def dft_registration_clipped(
    ...
    ----------
    buf1ft : torch.Tensor
        FFT of reference image (complex). DC in (0,0), not fftshifted.
    buf2ft : torch.Tensor
        FFT of image to register (complex). DC in (0,0), not fftshifted.
    usfac : int
        Upsampling factor. Registration precision = 1/usfac pixel.
    clip : int | tuple[int, int]
        Maximum shift in pixels. Scalar or (row_clip, col_clip).
    ...
    if usfac == 1:
        ...
        cc_real = cc.real
        _, flat_idx = torch.max(cc_real.reshape(-1), 0)
    ...
    c_start = n + 1 - n // 2 - 1
    c_end = c_start + n
    cc[r_start:r_end, c_start:c_end] = torch.fft.fftshift(buf1ft) * torch.conj(
        torch.fft.fftshift(buf2ft)
    )
...
def _dftups(
    ...
    usfac : int
        Upsampling factor.
    roff : float
        Row offset in upsampled grid.
    coff : float
        Column offset in upsampled grid.
    """
    nr, nc = inp.shape
    device = inp.device
    dtype = inp.dtype
