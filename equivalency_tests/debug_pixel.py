...
hp = raw - baseline
print(
    f"Baseline: NaN={np.sum(np.isnan(baseline))}, range=[{np.nanmin(baseline):.4f}, {np.nanmax(baseline):.4f}]"
)
print(
    f"Highpass: NaN={np.sum(np.isnan(hp))}, range=[{np.nanmin(hp):.4f}, {np.nanmax(hp):.4f}]"
)
...
print(
    f"Std (before max): NaN={np.sum(np.isnan(std_ts))}, range=[{np.nanmin(std_ts):.6f}, {np.nanmax(std_ts):.6f}]"
)
...
if np.any(zero_std):
    ...
    print(
        f"Z-score at those frames would be: {hp[zero_frames[:20]] / np.maximum(std_ts[zero_frames[:20]], 1e-10)}"
    )
...
print(
    f"|Z| > 10: {np.sum(np.abs(zscore) > 10)}, |Z| > 100: {np.sum(np.abs(zscore) > 100)}"
)
print(
    f"|Z| > 1000: {np.sum(np.abs(zscore) > 1000)}, |Z| > 1e6: {np.sum(np.abs(zscore) > 1e6)}"
)
...
with warnings.catch_warnings():
    warnings.simplefilter("ignore", RuntimeWarning)
    std_all = (
        moving_mad_noise(IMfden - IMb, window=baseline_window, axis=2) * denoise_window
    )
...
print(
    f"|Z| > 1000: {n_extreme} values out of {zscore_all.size} ({100 * n_extreme / zscore_all.size:.4f}%)"
)
...
if np.sum(bad_pixels) > 0:
    ...
    print("First 10 bad pixels (row,col,count):")
    for i in range(min(10, len(bad_r))):
        print(
            f"  ({bad_r[i]},{bad_c[i]}): {extreme_per_pixel[bad_r[i], bad_c[i]]} frames, "
    ...
    print(
        f"Bad pixels with nan_frac>0: {np.sum(bad_nan_fracs > 0)}/{np.sum(bad_pixels)}"
    )
...
for i in range(min(5, len(bad_r))):
    ...
    if rr < mat_std.shape[0] and cc < mat_std.shape[1]:
        print(
            f"  Pixel ({rr},{cc}): Python std={py_std_frame[rr, cc]:.8f}, "
