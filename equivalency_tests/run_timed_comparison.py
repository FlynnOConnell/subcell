...
print(
    f"Parameters: tau={tau:.2f}fr, sigma={sigma}, baseline_window={baseline_window}, denoise_window={denoise_window}"
)
...
print(
    f"Valid pixels: {np.sum(valid)} / {valid.size} ({100 * np.sum(valid) / valid.size:.1f}%)"
)
...
with timed("step1_moving_mean"), warnings.catch_warnings():
...
with timed("step2_moving_median"), warnings.catch_warnings():
    warnings.simplefilter("ignore", RuntimeWarning)
...
if np.sum(vb) > 0:
    ...
    print(
        f"    std_IM ratio (py/mat): mean={np.mean(ratio):.6f}, std={np.std(ratio):.6f}"
    )
...
print(
    f"    Python activity range: [{np.nanmin(activity_image):.6f}, {np.nanmax(activity_image):.6f}]"
)
print(
    f"    MATLAB activity range: [{np.nanmin(matlab_act_img):.6f}, {np.nanmax(matlab_act_img):.6f}]"
)
...
with timed("end_to_end_localize"), warnings.catch_warnings():
    warnings.simplefilter("ignore", RuntimeWarning)
...
if len(py_peaks_full.row) > 0 and len(matlab_peaks_r) > 0:
    ...
    for thr in thresholds:
        ...
        print(
            f"    Within {thr:.0f}px: {py_matched}/{len(py_coords)} Python, "
