...
print(
    f"Python  - min: {np.nanmin(py_act):.4f}, max: {np.nanmax(py_act):.4f}, median: {np.nanmedian(py_act):.4f}"
)
print(
    f"MATLAB  - min: {np.nanmin(matlab_act):.4f}, max: {np.nanmax(matlab_act):.4f}, median: {np.nanmedian(matlab_act):.4f}"
)
...
print(
    f"Total valid pix: {total_pix}, max_sources: {max_sources}, thresh_idx: {thresh_idx}"
)
...
print(
    f"Total valid pix: {total_pix_m}, max_sources: {max_sources_m}, thresh_idx: {thresh_idx_m}"
)
...
print(
    f"Python activity at border - mean: {np.nanmean(py_act[border]):.4f}, max: {np.nanmax(py_act[border]):.4f}"
)
print(
    f"MATLAB activity at border - mean: {np.nanmean(matlab_act[border & ~np.isnan(matlab_act)]):.4f}, max: {np.nanmax(matlab_act[border & ~np.isnan(matlab_act)]):.4f}"
)
...
print(
    f"Python activity at interior - mean: {np.nanmean(py_act[interior]):.4f}, max: {np.nanmax(py_act[interior]):.4f}"
)
print(
    f"MATLAB activity at interior - mean: {np.nanmean(matlab_act[interior]):.4f}, max: {np.nanmax(matlab_act[interior]):.4f}"
)
