...
print(
    f"Percentiles: 1%={np.percentile(valid_vals, 1):.4f}, 99%={np.percentile(valid_vals, 99):.4f}"
)
...
for fr in range(1, end_frame):
    ...
    if fr % 5000 == 0:
        print(
            f"  Frame {fr}: {n_peaks} peaks, activity max so far={np.max(activity_image):.4f}"
        )
...
print(
    f"Activity image (unnormalized) range: [{np.min(activity_image):.4f}, {np.max(activity_image):.4f}]"
)
...
print(
    f"Activity image (normalized) range: [{np.min(activity_image_norm):.4f}, {np.max(activity_image_norm):.4f}]"
)
...
print(
    f"Python activity_norm range: [{np.nanmin(activity_image_norm):.6f}, {np.nanmax(activity_image_norm):.6f}]"
)
...
if np.sum(vb) > 0:
    ...
    if len(finite_ratio) > 0:
        print(
            f"Ratio py/mat (where mat>1e-10): mean={np.mean(finite_ratio):.4f}, "
...
print(
    f"  DoG time series: NaN={np.sum(nan_ts)}, range=[{np.nanmin(dog_ts):.4f}, {np.nanmax(dog_ts):.4f}]"
)
