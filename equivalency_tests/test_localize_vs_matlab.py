...
if len(peaks.row) > 0:
    ax.scatter(peaks.col, peaks.row, s=40, facecolors='none',
...
ax.scatter(mat_peaks_col, mat_peaks_row, s=40, facecolors='none',
           edgecolors='r', linewidths=1.5)
...
vmax_m = np.nanpercentile(mat_act_clean, 99.5) if np.any(np.isfinite(mat_act_clean)) else 1
