...
if np.sum(vb) > 0:
    ...
    print(f"    std_IM ratio (py/mat): mean={np.mean(ratio):.4f}, std={np.std(ratio):.4f}, "
...
print(f"    Python activity range: [{np.nanmin(activity_image):.4f}, {np.nanmax(activity_image):.4f}]")
print(f"    MATLAB activity range: [{np.nanmin(matlab_act_img):.4f}, {np.nanmax(matlab_act_img):.4f}]")
...
if len(py_peaks_full.row) > 0 and len(matlab_peaks_r) > 0:
    ...
    for thr in thresholds:
        ...
        print(f"    Within {thr:.0f}px: {py_matched}/{len(py_coords)} Python matched, "
    ...
    if np.any(unmatched_py):
        print(f"    Python-only peaks (>3px from any MATLAB peak): {np.sum(unmatched_py)}")
    ...
    if np.any(unmatched_mat):
        print(f"    MATLAB-only peaks (>3px from any Python peak): {np.sum(unmatched_mat)}")
...
print(f"    Python mean image range: [{np.nanmin(py_mean):.1f}, {np.nanmax(py_mean):.1f}]")
