...
def create_initial_template(
    ...
    ----------
    frames_ds : np.ndarray
        Downsampled frames, shape (rows, cols, n_frames).
        Should be the sum across channels (single-channel representation).
    maxshift : int
        Maximum shift for padding and registration.
    min_cluster_size : int
        Minimum frames in a valid cluster.
    max_cutoff : float
...
def make_template_multi_trial(
    ...
    ----------
    mean_images : np.ndarray
        Stack of mean images, shape (rows, cols, n_trials).
    maxshift : int
        Maximum shift for alignment.
    ...
    tuple[np.ndarray, np.ndarray, np.ndarray]
        ...
        - template
        Aligned template with NaN padding.
        - motions
        Motion vectors (2, n_trials, n_trials).
        - corr_matrix
        Pairwise correlation coefficients (n_trials, n_trials).
    """
