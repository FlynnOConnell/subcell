...
def load_auto(path: str | Path) -> ExtractionData:
    ...
    ----------
    path : str | Path
        Path to a .zarr directory or .mat file.
    """
    path = Path(path)
    if path.suffix == ".mat":
        data = load_mat(path)
...
def load_zarr(zarr_path: str | Path) -> ExtractionData:
    ...
    if "initial_footprints" in summ["sources"]:
        initial_footprints = np.array(summ["sources"]["initial_footprints"]).astype(
            bool
        )
...
def load_mat(mat_path: str | Path) -> ExtractionData:
    ...
    try:
        ...
        try:
            ...
            logger.debug("Could not read analyzeHz from params", exc_info=True)
        ...
        if analyze_hz <= 1.0:
            try:
                ...
                logger.debug("Could not derive analyzeHz from aData", exc_info=True)
...
def _read_signal(f, grp, name: str, n_sources: int) -> np.ndarray:
    """Read one signal array from a MATLAB group, oriented (n_sources, n_time)."""
...
def _load_mat_trial(f, e_grp, n_sel: int, n_peaks: int) -> TrialExtraction | None:
    ...
    events = _read_signal(f, dF_target, "events", n_sources)
    denoised = _read_signal(f, dF_target, "denoised", n_sources)
    ls = _read_signal(f, dF_target, "ls", n_sources)
