...
def initialize_footprints(
    ...
    ----------
    source_rows : np.ndarray
        Row coordinates of source centers.
    source_cols : np.ndarray
        Column coordinates of source centers.
    sel_pix : np.ndarray
        2D boolean mask of selected pixels.
    sigma_px : float
        PSF Gaussian sigma.
    ...
    tuple[np.ndarray, np.ndarray, np.ndarray]
        ...
        - H
        Smoothed footprints, shape (n_selected_pixels, n_sources).
        - Hs
        Super-resolution footprints, shape (n_selected_pixels, n_sources).
        - H_valid
        Validity mask, shape (n_selected_pixels, n_sources).
    """
    h, w = sel_pix.shape
    ...
    for i in range(n_sources):
        ...
        tmp_hs[r, c] = 1.0
        tmp_hs_dilated = binary_dilation(tmp_hs > 0, structure=disk_se).astype(
            np.float64
        )
...
def smooth_hs_to_h(
    ...
    ----------
    Hs : np.ndarray
        Super-resolution footprints, (n_pixels, n_sources).
    sel_pix : np.ndarray
        2D boolean mask.
    h_filter : np.ndarray
